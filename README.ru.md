...
**Веб-дашборд:** выполните `python run_web_dashboard.py`, затем откройте в браузере [http://127.0.0.1:5001](http://127.0.0.1:5001/). Для живых данных с Arduino подключите плату по USB и настройте `pyserial` (см. раздел «Быстрый старт»).
...
Откройте `http://127.0.0.1:5001`: на **обложке** задайте сид (**Seed** / поле + **Add**) или нажмите **Start** (сид по умолчанию). После загрузки `/api/mission-data` откроется дашборд (3D-орбита, радар, показания, Arduino).
