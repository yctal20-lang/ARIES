...
<p align="center">
  ...
  Подзаголовок на веб-интерфейсе: <em>Autonomous Research & Intelligence Earth Satellite</em><br>
...
**Демо:** [https://a-r-i-e-s-1.onrender.com](https://a-r-i-e-s-1.onrender.com/) — развёрнуто на Render через Flask + Gunicorn.

> [!IMPORTANT]
> **Для запуска сайта необходимо подождать 3–5 минут.** Бесплатный инстанс Render уходит в спящий режим после ~15 минут бездействия. При первом открытии ссылки (или после периода неактивности) серверу нужно 3–5 минут, чтобы полностью проснуться и загрузить все зависимости. Это нормальное поведение для бесплатного хостинга.
...
| **Веб-дашборд** | Тёмная тема AetherOS: экран обложки (Seed / Add / Start), 3D орбита, радар, графики телеметрии, панель Arduino, предупреждения (в т.ч. по live-датчикам) |
...
Откройте `http://127.0.0.1:5000`: на **обложке** задайте сид (**Seed** / поле + **Add**) или нажмите **Start** (сид по умолчанию). После загрузки `/api/mission-data` откроется дашборд (3D-орбита, графики, радар, Arduino).
...
- **Центр** — **Trajectory**: 3D-сцена (Plotly.js) с Землёй, траекторией и мусором; счётчик преград; **попап по объекту** (размер, материал, тип, траектория; блок **ARDUINO LIVE** — расстояние, магнитное поле, температура, время до столкновения при наличии данных)
...
- **Нижний ряд графиков** — **Telemetry — ECI** (позиция), **Fusion & Velocity**, **Resources** (топливо, мусор, уровень опасности)
