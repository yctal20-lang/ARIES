...
python run_web_dashboard.py --arduino-port COM3  # Windows serial port (optional)
```
...
- **Center** — **Trajectory**: 3D scene (Plotly.js) with Earth, path, debris; obstacle count; **debris popup** (size, material, type, trajectory; **ARDUINO LIVE** — distance, magnetic field, temperature, time-to-collision when data is available)
